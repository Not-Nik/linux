...
HvFlushVirtualAddressList(Ex). When the flag is set, a vCPU attempting this will
be suspended and will only resume once the bit is cleared again using this
ioctl. During suspension, the vCPU will not react to interrupts or other events.
Because it is caused by a hypercall, the suspension naturally happens on a guest
instruction boundary. This behaviour and the suspend state itself are specified
in Microsoft's "Hypervisor Top Level Functional Specification" (TLFS).
